"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.POST = exports.dynamic = exports.runtime = void 0;
var server_1 = require("next/server");
var zod_1 = require("zod");
var crypto_1 = require("crypto");
var drizzle_orm_1 = require("drizzle-orm");
var db_1 = require("@/config/db");
var schema_1 = require("@/config/schema");
var mail_1 = require("@/lib/mail"); // ⬅️ NEW
exports.runtime = "nodejs";
exports.dynamic = "force-dynamic";
function json(data, status) {
    if (status === void 0) { status = 200; }
    return server_1.NextResponse.json(data, {
        status: status,
        headers: { "Cache-Control": "no-store" }
    });
}
var ForgotBody = zod_1.z.object({
    email: zod_1.z.string().email()
});
function normalizeEmail(email) {
    return email.trim().toLowerCase();
}
// Simple 6-digit numeric OTP
function generateOtp() {
    var n = Math.floor(100000 + Math.random() * 900000); // 100000–999999
    return String(n);
}
function hashOtp(otp) {
    return crypto_1["default"].createHash("sha256").update(otp).digest("hex");
}
function POST(req) {
    return __awaiter(this, void 0, void 0, function () {
        var bodyUnknown, parsed, emailNorm, user, otp, otpHash, now, expiresAt, err_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, req.json()["catch"](function () { return null; })];
                case 1:
                    bodyUnknown = _a.sent();
                    parsed = ForgotBody.safeParse(bodyUnknown);
                    if (!parsed.success) {
                        return [2 /*return*/, json({ error: "Invalid email" }, 400)];
                    }
                    emailNorm = normalizeEmail(parsed.data.email);
                    return [4 /*yield*/, db_1.db.query.usersTable.findFirst({
                            where: drizzle_orm_1.eq(schema_1.usersTable.email, emailNorm)
                        })];
                case 2:
                    user = _a.sent();
                    if (!user) {
                        // Still return ok, but do nothing. Frontend shows generic success.
                        return [2 /*return*/, json({ ok: true })];
                    }
                    otp = generateOtp();
                    otpHash = hashOtp(otp);
                    now = new Date();
                    expiresAt = new Date(now.getTime() + 15 * 60 * 1000);
                    // Optionally: clear old tokens for this user/email (not required, but cleaner)
                    return [4 /*yield*/, db_1.db["delete"](schema_1.passwordResetTokensTable)
                            .where(drizzle_orm_1.eq(schema_1.passwordResetTokensTable.userId, user.id))];
                case 3:
                    // Optionally: clear old tokens for this user/email (not required, but cleaner)
                    _a.sent();
                    // Store OTP
                    return [4 /*yield*/, db_1.db.insert(schema_1.passwordResetTokensTable).values({
                            userId: user.id,
                            email: emailNorm,
                            otpHash: otpHash,
                            expiresAt: expiresAt
                        })];
                case 4:
                    // Store OTP
                    _a.sent();
                    _a.label = 5;
                case 5:
                    _a.trys.push([5, 7, , 8]);
                    return [4 /*yield*/, mail_1.sendPasswordResetEmail(emailNorm, otp)];
                case 6:
                    _a.sent();
                    return [3 /*break*/, 8];
                case 7:
                    err_1 = _a.sent();
                    console.error("Error sending password reset email:", err_1);
                    // Generic error so we don't leak details
                    return [2 /*return*/, json({ error: "Could not send reset email. Please try again." }, 500)];
                case 8:
                    // Keep this for local debugging if you want
                    console.log("Password reset OTP for " + emailNorm + ": " + otp);
                    return [2 /*return*/, json({ ok: true })];
            }
        });
    });
}
exports.POST = POST;
